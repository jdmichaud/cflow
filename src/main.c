...
#define GROUP_ID 0
     ...
     { "exclude", 'u', N_("NAME"), 0,
       N_("Exclude symbols from output"),
       GROUP_ID+1 },
     { "format", 'f', N_("NAME"), 0,
...
parse_opt (int key, char *arg, struct argp_state *state)
{
     ...
     // Find end of array
     int i = 0;
     while (excluded_symbols[i] != 0 || excluded_symbols[i + 1] != 0) {
       if (i > 1000) {
         error(EX_USAGE, 0, _("filter: too many symbols, increase static array size"));
       }
       ++i;
     }
     if (i != 0) {
     ...
     memcpy(&excluded_symbols[i], arg, strnlen(arg, 1024 - i));
    break;
     case 'f':
...
int
is_excluded(const char *symbol_name) {
  // if the first two character are 0, then there is not filter
  if (excluded_symbols[0] == 0 && excluded_symbols[1] == 0) {
    return 1;
  }

  int i = 0;
  while (i < 1024 && excluded_symbols[i] != 0 && excluded_symbols[i + 1] != 0) {
    /* printf("cmp %s %s\n", symbol_name, &excluded_symbols[i]); */
    if (strncmp(symbol_name, &excluded_symbols[i], 1024) == 0) {
      /* printf("OK\n"); */
      return 1;
    }
    while (i < 1024 && excluded_symbols[i] != 0 && excluded_symbols[i + 1] != 0) {
